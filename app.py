...
def get_user_stats(name):
    ...
    if total >= 125: form, time_limit, max_lives = "最終形態", 30, 7
    elif total >= 80: form, time_limit, max_lives = "第４形態", 15, 4
    elif total >= 50: form, time_limit, max_lives = "第３形態", 10, 3
    elif total >= 20: form, time_limit, max_lives = "第２形態", 7, 2
    else: form, time_limit, max_lives = "第１形態", 5, 1
...
@app.route('/')
def index():
    """TOPアクセス時、ログイン済みならBGM付き外枠(game_shell)へ飛ばす"""
    if 'user_name' not in session:
        return redirect(url_for('login'))
    return redirect(url_for('game_shell'))

@app.route('/game_shell')
def game_shell():
    """BGMを管理する親画面。iframeの中にゲーム本体を表示する"""
    if 'user_name' not in session:
        return redirect(url_for('login'))
    # iframe内に最初に表示するページを指定
    target = request.args.get('next', url_for('index_content'))
    return render_template('layout.html', target_url=target)
...
def start_quiz(category):
    ...
    random.shuffle(ids)
    session.update({'question_ids': ids, 'current_index': 0, 'score': 0, 'new_solved_count': 0, 'mode': 'study', 'category': category})
...
def start_total_study():
    ...
    session.update({'question_ids': [q['id'] for q in all_q[:20]], 'current_index': 0, 'score': 0, 'new_solved_count': 0, 'mode': 'total_study', 'category': "総合腕試し"})
    return redirect(url_for('quiz_page'))
...
def end_logic(name, score, total, failed, enemy_id=1):
    ...
    return render_template('result.html', score=score, total=total, new_solved_count=session.get('new_solved_count', 0), user_stats=get_user_stats(name), failed=failed, enemy_id=enemy_id, mode=session.get('mode'))
